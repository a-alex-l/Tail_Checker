...
get_delimeter <- function(percentile) {
  delimeter_by_data <- function(data) {
    ...
    delimiters <- q_gpd(unlist(data[1:p1]) , package="extRemes", method="Lmoments", truncate=treshold_global.trsh, probs=petcentiles) * 0.5 + 
      q_gpd(unlist(data[p1:p2]), package="extRemes", method="GMLE", truncate=treshold_global.trsh, probs=petcentiles) * 0.5
    sd <- delimiters[601] - delimiters[201]
    weights <- delimiters[2:n] - delimiters[1:n - 1]
    ans <- get_best_delimeter(1, n - 1, weights,
                              delimiters[2:n], petcentiles[2:n],
                              percentile, sd=sd)
    return(unlist(ans))
  }
}