...
get_delimeter <- function(percentile) {
  delimeter_by_data <- function(data) {
    ...
    delimeters <- q_gpd(unlist(data) , package="extRemes", method="Lmoments", truncate=treshold_global.trsh, probs=petcentiles) * 0.5 + 
      q_gpd(unlist(data), package="extRemes", method="GMLE", truncate=treshold_global.trsh, probs=petcentiles) * 0.5
